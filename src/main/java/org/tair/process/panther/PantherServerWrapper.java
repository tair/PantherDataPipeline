...
//            System.out.println("Prop file not found!");
    ...
    public List<String> callOrtholog_uniprot(String uniprotId, Map<String, String> uniprot2agi_mapping, Map<String, List<String>> organisms_mapping) throws Exception {
...
//            System.out.println(organism_names);
...
//        System.out.println(orthologs.toString());
        StringBuilder orthoTxtSb = new StringBuilder(String.join("\t", "Organism", "UniProt ID", "Gene ID", "Ortholog Type\n"));
        for(int i=0; i<orthologs.size(); i++) {
            OrthoMapped mapped = orthologs.get(i);
            ...
            orthoTxtSb.append(String.join("\t", full_organism, mapped.getUniprot_id(), mapped.getTarget_gene_id(), mapped.getOrtholog() + "\n"));
        }
...
//            String organism2 = o2.getOrganism();
    ...
    public List<String> callParalog_uniprot(String uniprotId, Map<String, String> uniprot2agi_mapping, Map<String, String> agi2symbol_mapping) throws Exception {
            ...
            for(int i=0; i<paralogs.size(); i++) {
                ...
                if ( agi_id != null && agi_id.contains("/")) {
                    ...
                    for (String agi_id_item : agi_id_list ){
                        ...
                        Mapped para_obj_temp = objectMapper.readValue(objectMapper.writeValueAsString(para_obj), Mapped.class);
...
//                System.out.println(agi_id + " " + primarySymbol);
            ...
            e.printStackTrace();
            System.out.println(String.join(" ","error downloading paralogs:", uniprotId, uniprot2agi_mapping.get(uniprotId),jsonString));
            return null;
        }
    }
