...
public class pantherToPhyloXmlPipeline {
    ...
    public static void main(String args[]) {
        ...
        // convertAllInDirectory(); // converts all panther json files in
        // pruned_panther_files directory
    ...
    static void PantherJsonToPhyloXml(String fileName) {
        ...
        if (panther.getSearch() != null) {
            ...
            // transformation pantherForPhylo object to phyloxml object, build phyloxml tree
    ...
    // Sets clade attributes equal to module.pantherForPhylo.Annotation attributes
    // but only if they are needed so XML file looks correct.
    // So we only allocate memory for objects when we see the
    // module.pantherForPhylo.Annotation node attributes are not empty.
    ...
    // instead of printing when visiting the node, we set clade node attributes
    // equal to annotation
    // node attributes.
    static void constructPhyloTreeWithAnnoTree(Annotation root, Clade croot) {
        ...
        while (!anno_q.isEmpty()) {
            ...
            while (n > 0) {
                ...
                // without this, once you get to leaf node, you'll try to access thru a null
                // object
                if (annoTempNode.getChildren() != null) {
                    ...
                    for (int i = 0; i < annoTempNode.getChildren().getAnnotation_node().size(); i++) {
                        cladeTempNode.getClade().add(new Clade()); // adds n childs based on the annotation nodes amount
                                                                   // of childs
    ...
    // Given a module.phyloxml.Phyloxml object, creates a local xml file formatted
    // to be easier to read. Change the number in
    // the parameter to change the amount of indent space
    // https://stackoverflow.com/questions/46708498/jaxb-marshaller-indentation
    // fixed indent bug looking at this
