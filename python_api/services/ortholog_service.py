...
class OrthologService:
    ...
    def _load_organism_mapping(self) -> Optional[Dict[str, str]]:
        ...
        try:
            # Try to load from webapp resources first
            mapping_file = "src/main/webapp/WEB-INF/organism_to_display.csv"
            if not os.path.exists(mapping_file):
                # Fallback to python_api directory
                mapping_file = "python_api/data/organism_to_display.csv"
                if not os.path.exists(mapping_file):
                    self.logger.warning("Organism mapping file not found")
                    return None
